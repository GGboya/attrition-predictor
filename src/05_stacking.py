"""Phase 5 — OOF-tuned stacking (current production; formerly v3).

Two targeted improvements over the legacy v2 (`05_legacy_stack_v2.py`):

1) Seed-averaged MT-MLP.  Single-seed MT-MLP has non-trivial variance from
   random initialisation + mini-batch order.  Legacy-v2 stack-LR pulled
   weight 0.26 onto MT-MLP, so reducing its variance should tighten the
   stacked prediction.  We train 5 seeds per fold and average their probs.

2) Fifth base learner: ExtraTrees.  Uses fully random splits (not best
   splits) — a different tree bias than RF/XGB/NRBoost.  Should expose a
   bit more orthogonal signal for the meta.

Five bases total:
    RF, NR-Boost (q=0.7), MT-MLP (seed-avg), SVM-RBF, ExtraTrees.

Meta-learners: L2 logistic on logit(OOF), convex combo on prob(OOF).

Outputs:
    src/tables/table9c_stack_v3_panel.csv
    src/tables/table9c_stack_v3_weights.csv
    src/tables/table9c_stack_v3_delong.csv
    src/tables/table9c_stack_v3_corr.csv
    src/figures/fig9c_stack_v3_roc.png
"""
...
INTENT = "离职意向"
N_FOLDS = 5
N_MT_SEEDS = 5  # seeds to average MT-MLP over per fold
...
print(f"clean {df.shape}  train {len(train_idx)}  test {len(test_idx)}  "
      f"features: {len(feat_cols)} (no intent)  MT seeds: {N_MT_SEEDS}")
...
def fit_predict_mtmlp_seedavg(Xs_tr, y_bin_tr, y_ord_tr, Xs_pred, *, in_dim,
    ...
    """Train n_seeds MT-MLPs, average their predicted probabilities."""
...
corr_df.to_csv(OUT_TABLES / "table9c_stack_v3_corr.csv")
...
# v2 setting
meta_lr, oof_meta_lr, test_meta_lr = fit_meta_lr(C=1.0, class_weight="balanced")
# less regularisation
meta_lr_hC, oof_meta_lr_hC, test_meta_lr_hC = fit_meta_lr(C=10.0, class_weight=None)

print(f"\n  stacked-LR (C=1, bal)    CV AUC={roc_auc_score(y_tr, oof_meta_lr):.4f}  "
      f"test AUC={roc_auc_score(y_te, test_meta_lr):.4f}  "
      f"weights={dict(zip(BASES, np.round(meta_lr.coef_.ravel(), 3)))}")
print(f"  stacked-LR (C=10, none)  CV AUC={roc_auc_score(y_tr, oof_meta_lr_hC):.4f}  "
      f"test AUC={roc_auc_score(y_te, test_meta_lr_hC):.4f}  "
      f"weights={dict(zip(BASES, np.round(meta_lr_hC.coef_.ravel(), 3)))}")
...
# rank-averaged ensemble: average of base rank(OOF) positions
def to_rank(p):
    order = np.argsort(p); r = np.empty_like(order, dtype=float)
    r[order] = np.arange(len(p))
    return r / max(1, len(p) - 1)

rank_oof = np.column_stack([to_rank(oof[k]) for k in BASES]).mean(axis=1)
rank_test = np.column_stack([to_rank(test_preds[k]) for k in BASES]).mean(axis=1)

oof_mean = P_tr.mean(axis=1); test_mean = P_te.mean(axis=1)

oof_probs = {**oof,
             "Stack_LR":      oof_meta_lr,
             "Stack_LR_hC":   oof_meta_lr_hC,
             "Stack_CVX":     oof_meta_cvx,
             "MeanAvg":       oof_mean,
             "RankAvg":       rank_oof}
test_probs = {**test_preds,
              "Stack_LR":     test_meta_lr,
              "Stack_LR_hC":  test_meta_lr_hC,
              "Stack_CVX":    test_cvx,
              "MeanAvg":      test_mean,
              "RankAvg":      rank_test}

name_map = {"RF": "RandomForest", "NRBoost": "NR-Boost (q=0.7)",
            "MTMLP": "MT-MLP (seed-avg)", "SVM": "SVM-RBF",
            "ET": "ExtraTrees",
            "Stack_LR": "Stacking — L2 LR (bal)",
            "Stack_LR_hC": "Stacking — L2 LR (C=10)",
            "Stack_CVX": "Stacking — convex combo",
            "MeanAvg": "Mean-avg 5 bases",
            "RankAvg": "Rank-avg 5 bases"}
...
panel_df.to_csv(OUT_TABLES / "table9c_stack_v3_panel.csv", index=False)
print(f"\nwrote {OUT_TABLES / 'table9c_stack_v3_panel.csv'}")
print(panel_df.to_string(index=False,
      formatters={c: "{:.3f}".format for c in panel_df.select_dtypes("float").columns}))

w_df = pd.DataFrame({
    "base": BASES,
    "stack_LR_bal_coef": meta_lr.coef_.ravel(),
    "stack_LR_C10_coef": meta_lr_hC.coef_.ravel(),
    "convex_weight": w_cvx,
    "convex_weight_fold_mean": np.mean(fold_weights, axis=0),
    "convex_weight_fold_std":  np.std(fold_weights, axis=0),
})
w_df.to_csv(OUT_TABLES / "table9c_stack_v3_weights.csv", index=False)
print(f"wrote {OUT_TABLES / 'table9c_stack_v3_weights.csv'}")
print(w_df.to_string(index=False,
      formatters={c: "{:.4f}".format for c in w_df.select_dtypes("float").columns}))

delong_rows = []
for champ in ["Stack_LR", "Stack_LR_hC", "Stack_CVX", "MeanAvg", "RankAvg"]:
    cp = cal[champ]
...
dlg.to_csv(OUT_TABLES / "table9c_stack_v3_delong.csv", index=False)
print(f"\nwrote {OUT_TABLES / 'table9c_stack_v3_delong.csv'}")
# only show champ-vs-RF rows + best ΔAUC rows
dlg_show = dlg[dlg.baseline == "RandomForest"]
print(dlg_show.to_string(index=False,
      formatters={c: "{:.4f}".format for c in dlg_show.select_dtypes("float").columns}))

fig, axes = plt.subplots(1, 2, figsize=(12, 4.8))
order = ["SVM", "MTMLP", "ET", "NRBoost", "RF", "MeanAvg", "RankAvg",
         "Stack_CVX", "Stack_LR_hC", "Stack_LR"]
colors = {"Stack_LR": "crimson", "Stack_LR_hC": "firebrick",
          "Stack_CVX": "darkorange", "RF": "steelblue", "RankAvg": "seagreen"}
for k in order:
    p = cal[k]
    fpr, tpr, _ = roc_curve(y_te, p)
    is_champ = k.startswith("Stack") or k == "RankAvg"
    lw = 2.5 if is_champ else 1.2
    alpha = 1.0 if is_champ or k == "RF" else 0.45
    c = colors.get(k, None)
...
plt.tight_layout()
fig_path = OUT_FIGS / "fig9c_stack_v3_roc.png"
